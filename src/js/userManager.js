...
export default class UserManager {
  ...
  async renderLoginForm() {
    this.clearParent();
    loadTemplate("../partials/login-form.html")
      .then((template) => {
  ...
  async renderResgisterForm() {
    this.clearParent();
    loadTemplate("../partials/register-form.html")
      .then((template) => {
        renderWithTemplate(template, this.parent);
      })
  ...
  async renderUserInfo(id = null) {
    ...
    this.clearParent();
    loadTemplate("../partials/user-info.html")
      .then((template) => {
        renderWithTemplate(template, this.parent);
      })
      .then(() => {
  ...
  attachRegisterListeners() {
    document
      ...
      .addEventListener("submit", async (e) => {
        ...
        let response = await this.dataSource.register(
          fnameInput.value,
          lname.value,
          dname.value,
          email.value,
        ...
        if (response.error) {
          ...
          this.renderLoginForm().then(() =>
            alertMessage("Welcome to the app! Please login in now", "success")
          );
        }
      });
  }
    ...
    document
      ...
      .addEventListener("submit", async (e) => {
        ...
        const blog = {
          ...
          summary:
            contentInput.value.length > 100
              ? contentInput.value.substring(0, 100) + "..."
              : contentInput.value,
        ...
        if (response.error) {
          ...
          this.renderUserInfo().then(() =>
            alertMessage("Blogging Success!", "success")
          );
